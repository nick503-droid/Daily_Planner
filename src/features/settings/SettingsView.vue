...
// initialValue: 'light' => la primera vez que abre la app, siempre arranca en modo claro.
// Después de que el usuario lo cambie, se guarda su preferencia en localStorage.
const isDark = useDark({ initialValue: 'light' })
const toggleDark = useToggle(isDark)
</script>

<template>
  <div class="flex flex-col h-screen bg-slate-50 dark:bg-[#1a1a2e] z-50 transition-colors duration-300">
    
    <header class="flex items-center px-4 py-4 bg-white dark:bg-[#16213e] border-b border-slate-200 dark:border-slate-700/30">
    ...
    <main class="flex-1 p-4 overflow-y-auto space-y-6">
      ...
      <div class="space-y-2">
        ...
        <div class="bg-white dark:bg-[#16213e] rounded-xl shadow-sm border border-slate-100 dark:border-slate-700/40 p-4 flex gap-3">
          <button
            @click="setLocale('es')"
        ...
        <div class="bg-white dark:bg-[#16213e] rounded-xl shadow-sm border border-slate-100 dark:border-slate-700/40 p-4">
          <div class="flex items-center justify-between" @click="toggleDark()">
            ...
            <div 
              ...
              :class="isDark ? 'bg-blue-600' : 'bg-slate-300 dark:bg-slate-500'"
            >
              <div 
      ...
      <div class="space-y-2">
        ...
        <div class="bg-white dark:bg-[#16213e] rounded-xl shadow-sm border border-slate-100 dark:border-slate-700/40 p-4 opacity-50">
          <div class="flex items-center gap-3">
            <span class="text-slate-400">
